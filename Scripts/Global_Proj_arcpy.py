#####
#CAMELS Linear Regression Models: GLOBAL
#Written by: Logan Qualls, email: lmqualls@crimson.ua.edu
#Finalized on: November 29, 2020
#Loops through precipitation and streamflow data for CAMELS basins (gauges) in all 18 hydrological regions
#and creates linear models for those gauges; stores all r-squared values representing the comparison between predicted
#data and observed streamflow data; creates attribute field for r-squared value and fills that field with corresponding
#r-squared values of the linear model run for that gauge
#Parameters include: (0) workspace, (1) path to "maurer" folder, (2) path to "usgs_streamflow" folder,
#(3) path to "HCDN_nhru_final_671" shapefile
#####
...
for region in regions:
    maurer = arcpy.GetParameterAsText(1)
    usgs = arcpy.GetParameterAsText(2)
    ...
    for gauge in gauges:
        ...
        if xtrain.shape[0] < 100 or xtest.shape[0] < 100:
            print(region, ", ", gauge, ": not enough data for prediction")
            continue
